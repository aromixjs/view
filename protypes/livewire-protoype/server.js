...
class View {
  ...
  static #serializeProps(props) {
    ...
    for (let [k, v] of Object.entries(props)) {
      out[k] = typeof v === "function" && v._actionName ? { __wireAction: v._actionName } : v;
    }
    return out;
  }
  ...
  static #resolveProps(props, parent) {
    ...
    for (let [k, v] of Object.entries(props || {})) {
      out[k] =
        v && typeof v === "object" && "__wireAction" in v
          ? parent
            ? parent.wire[v.__wireAction]
            : undefined
          : v;
    }
    return out;
  ...
  static renderToHtml(node, hostView, snapshots = new Map(), isRoot = true) {
    ...
    if (typeof node.tag === "function" && node.tag.prototype instanceof View) {
      ...
      let html = View.renderToHtml(
        childView.render.call(childView.wire),
        childView,
        snapshots,
        true,
      );
    ...
    for (let [k, v] of Object.entries(rest)) {
      if (k.startsWith("on") && typeof v === "function")
        wireAttrs += ` data-wire-${k.slice(2).toLowerCase()}="${v._actionName}"`;
      else htmlAttrs += ` ${k}="${v}"`;
    }
    if (isRoot) {
    ...
    let inner = children
      .map((c) => (typeof c === "string" ? c : View.renderToHtml(c, hostView, snapshots, false)))
      .join("");
  ...
  static update({ snapshot, parentSnapshot, calls }) {
    ...
    for (let call of calls) {
      if (typeof targetView[call.method] !== "function")
        throw new Error(`unknown method "${call.method}" on ${TargetClass.name}`);
  ...
  static register(c) {
    View.registry.set(c.name, c);
  }
  static lookup(n) {
    return View.registry.get(n);
  }
}
...
class UserCard extends View {
  ...
  render() {
    return {
      tag: "div",
      style:
        "display:flex;justify-content:space-between;align-items:center;padding:8px;border:1px solid #ddd;margin:6px 0;border-radius:6px",
      children: [
  ...
  render() {
    return {
      ...
      children: [
        ...
        ...this.users.map((u) => ({
          tag: UserCard,
          props: { userId: u.id, userName: u.name, onDelete: this.remove },
        })),
      ],
    };
  }
