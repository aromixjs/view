...
const { App, Header, Sidebar, SearchBar, CartBadge, WishlistPanel, Toast, ProductGrid, ProductCategory, ProductCard } = registry
...
app.get('/', (c) => {
  ...
  let CATALOG = {
    Shoes: [{ productId: 1, name: 'Runner', price: 80, stock: 2 }, { productId: 2, name: 'Sandal', price: 40, stock: 10 }],
    Bags:  [{ productId: 3, name: 'Backpack', price: 60, stock: 5 }, { productId: 4, name: 'Tote', price: 35, stock: 5 }],
  }
  ...
  let categoriesHtml = Object.entries(CATALOG).map(([categoryName, products], ci) => {
    ...
    let cardsHtml = products.map((p) => {
      ...
      let onAddProvenance = { component: 'ProductCategory', ref: categoryRef, action: 'recordAdd', reads: { addCount: 0, __v: 1 } }
      let card = ProductCard({ ...p, qty: 1, onAdd: () => {} }) // real fn unused at SSR time, render doesn't call actions
      let dataProps = { productId: { value: p.productId }, name: { value: p.name }, price: { value: p.price }, stock: { value: p.stock }, onAdd: onAddProvenance }
      return wrap(ProductCard, card, card.html(), `card-${p.productId}`, ` data-props="${escapeAttr(JSON.stringify(dataProps))}"`)
    }).join('')
  ...
  let meta = Object.fromEntries(Object.entries(registry).map(([name, Factory]) => [name, Factory({}, '').meta]))
...
app.post('/action', async (c) => {
  ...
  if (!Factory) return c.json({ error: { code: 'UNKNOWN_COMPONENT', message: body.component } }, 404)
  ...
  if (conflictVersion !== null) {
    return c.json({ conflict: { ref: body.ref, current: { __v: conflictVersion } } })
  }

  let ctx = { calls: body.calls || {}, session: { isVIP: !!c.req.header('x-vip') }, writes: {}, errors: {} }
  ...
  if (Object.keys(result.changed).length) {
    ...
    ctx.writes[body.ref] = { ...(ctx.writes[body.ref] || {}), ...result.changed }
  }
