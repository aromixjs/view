  ...
  const meta = {
    ...
    actions: {
      addItem: {
        reads: ["itemCount"],
        params: ["qty"],
        calls: [{ kind: "trigger", as: "./Toast", method: "show" }],
      },
    },
  };
...
function SearchBar() {
  ...
  const html = () =>
    `<input data-bind="query" value="${query}"><button data-action="search">Search</button>`;
  const meta = {
    state: ["query"],
    props: {},
    actions: {
      search: {
        reads: [],
        params: ["term"],
        calls: [{ kind: "trigger", as: "./ProductGrid", method: "setFilter" }],
      },
    },
  };
...
function ProductCard(props) {
  ...
  function add() {
    if (qty > props.stock) {
      return {
        error: { code: "OUT_OF_STOCK", message: `Only ${props.stock} left of ${props.name}` },
      };
    }
  ...
  const meta = {
    ...
    actions: {
      ...
      save: {
        reads: ["productId"],
        params: [],
        calls: [{ kind: "trigger", as: "./WishlistPanel", method: "addToWishlist" }],
      },
    },
  };
...
function Header(cartBadgeHtml, searchBarHtml) {
  return {
    html: () => `<header>${searchBarHtml}<strong>Shop</strong> ${cartBadgeHtml}</header>`,
    meta: { state: [], props: {}, actions: {} },
  };
}
function Sidebar(wishlistHtml) {
  return {
    html: () => `<aside>${wishlistHtml}</aside>`,
    meta: { state: [], props: {}, actions: {} },
  };
}
function App(headerHtml, gridHtml, sidebarHtml, toastHtml) {
  return {
    html: () => `${headerHtml}${gridHtml}${sidebarHtml}${toastHtml}`,
    meta: { state: [], props: {}, actions: {} },
  };
}
